...
<template>
  ...
  <aside
    ...
    <div
      ...
      :class="
        isSidebarExpanded ? 'px-6 justify-between' : 'justify-center px-0'
      ">
      <Link
        :href="route('home')"
        ...
        <span
          ...
          class="font-display font-bold text-xl text-slate-900 whitespace-nowrap transition-opacity duration-300">
      ...
      <button
        @click="emit('closeMobileMenu')"
        class="lg:hidden p-1 text-slate-400 hover:text-slate-600 transition-colors">
        <X class="w-5 h-5" />
      </button>
    </div>
      ...
      <template v-for="item in menuItems" :key="item.route">
        <Link
          ...
          <div
            ...
            class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-violet-600 rounded-r-full"></div>
        </Link>
      </template>
    </div>
